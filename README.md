...
* [Ansible](https://www.ansible.com) (["What is Ansible?"](https://www.youtube.com/watch?v=fHO1X93e4WA)) is free and open-source software from Red Hat that automates tasks on servers. For example, it can configure servers after they are provisioned (although it can do that indirectly too).
* It uses Secure Shell (SSH) to connect to these servers (only the local workstation/controller needs to have Ansible installed, something that differentiates it from other DevOps tools like Chef). 
